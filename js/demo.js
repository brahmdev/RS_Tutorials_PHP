jQuery(document)
		.ready(
				function($) {
					...
					jQuery("#stylechooser  #styleToggle")
							.click(
									function() {
										debugger;
										setCookie(
												'ct_macaroon.switcher',
												$chooser.css('left') == '0px'
														|| $chooser.css('left') == '-1px' ? 'hide'
														: 'show');
										hideStyleswitcher();
										return false;
									});
					...
					jQuery('#stylechooser [data-value]')
							.click(
									function(event) {
										var a = jQuery(this);
										if (a.attr('data-dependent')) {
											jQuery(a.attr('data-dependent'))
													.click();
										}
										var c = a.attr('data-value');
										var off = $body.hasClass(c)
												&& !a.hasClass('btn');
										jQuery('[data-value]',
												a.parents('div.mkSpace'))
												.each(
														function() {
															var t = jQuery(this)
																	.removeClass(
																			'demoActive btn-primary');
															$body
																	.removeClass(t
																			.attr('data-value'));
														});
										if (off) {
											...
											if (a
													.attr('data-clear-container-deselected') !== undefined) {
												jQuery(
														a
																.attr('data-clear-container-deselected'))
														.find('.demoActive')
														.toggleClass(
																'demoActive');
											}
					...
					function store() {
						var r = '';
						jQuery(
								'a[data-value].btn-primary, [data-value].demoActive',
...
function selectDropdown() {
	if (getCookie('ct_macaroon.dropdown') != null) {
		jQuery('.navbar-nav').removeClass('ct-navbar--fadeInUp');
